...
class Roket(pygame.sprite.Sprite):
    ...
    def __init__(self, posisi: Posisi, genom: Genom) -> None:
        ...
        self.warna: tuple[int, int, int] = Warna.HITAM
        self.kecepatan: int = 10
        self.rect: pygame.Rect = pygame.Rect(
            self.posisi[0],
            self.posisi[1],
            Konfigurasi.LEBAR_ROKET,
            Konfigurasi.TINGGI_ROKET,
        )
    ...
    def di_luar_batas(self) -> bool:
        if self.rect.x <= 0 or self.rect.x + self.rect.width >= Konfigurasi.LEBAR_LAYAR:
            return True
        if (
            self.rect.y <= 0
            or self.rect.y + self.rect.height >= Konfigurasi.TINGGI_LAYAR
        ):
            return True
        return False
